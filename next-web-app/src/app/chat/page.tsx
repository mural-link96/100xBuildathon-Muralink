...
const DesignAgentChat = () => {
    ...
    }, [chatSessions, currentSessionId, products, generatedImages, isGenerating, isGeneratingImage]);
    ...
    const toggleProductExpanded = (productIndex: number) => {
    ...
    const initializeDefaultSelections = (newProducts: Product[]) => {
        ...
        for (let i = 0; i < newProducts.length; i++) {
            ...
            if (product.shopping_search?.shopping_results && product.shopping_search.shopping_results.length > 0) {
                ...
                defaultExpanded[i] = true; // All expanded by default
            }
        }
    ...
    return (
        <div className="mt-6 p-4 sm:p-6 bg-white rounded-2xl shadow-lg border border-gray-100 max-w-80 md:max-w-xl lg:max-w-3xl min-w-0 overflow-hidden">
            <h3 className="text-lg font-semibold text-gray-800 mb-4 flex items-center">
                ...
                <span className="truncate">Recommended Products</span>
            </h3>
            ...
            <div className="space-y-4">
                {products.map((product, productIndex) => {
                    ...
                    return (
                        <div key={productIndex} className="border border-gray-200 rounded-xl overflow-hidden min-w-0">
                            {/* Product Header - Clickable to toggle */}
                            <button
                                onClick={() => toggleProductExpanded(productIndex)}
                                className="w-full px-4 py-3 bg-gray-50 hover:bg-gray-100 transition-colors flex items-center justify-between min-w-0"
                            >
                                <div className="flex items-center space-x-2 sm:space-x-3 min-w-0 flex-1">
                                    <div className="text-sm font-medium text-gray-700 truncate">
                                        Product {productIndex + 1}
                                        {product.name && (
                                            <span className="text-sm text-gray-500 ml-1 sm:ml-2">({product.name})</span>
                                        )}
                                    </div>
                                    <span className="text-xs text-gray-400 bg-gray-200 px-2 py-1 rounded-full flex-shrink-0">
                                        {product.shopping_search.shopping_results.length}
                                    </span>
                                    {selectedItems[productIndex] !== undefined && (
                                        <span className="text-xs text-green-600 bg-green-100 px-2 py-1 rounded-full flex-shrink-0 hidden sm:inline">
                                            Selected
                                        </span>
                                    )}
                                </div>
                                ...
                                <svg 
                                    className={`w-5 h-5 text-gray-500 transition-transform duration-200 flex-shrink-0 ml-2 ${isExpanded ? 'rotate-180' : ''}`}
                                    fill="none" 
                                    stroke="currentColor" 
                                    viewBox="0 0 24 24"
                                >
                            ...
                            <div className={`transition-all duration-300 overflow-hidden ${isExpanded ? 'max-h-96' : 'max-h-0'}`}>
                                <div className="p-4">
                                    <div className="w-full overflow-x-auto">
                                        <div className="flex gap-3 pb-2 min-w-max" style={{ scrollbarWidth: 'thin' }}>
                                            {product.shopping_search.shopping_results.map((item, itemIndex) => {
                                                ...
                                                return (
                                                    <div
                                                        ...
                                                        className={`relative group cursor-pointer transition-all duration-200 transform rounded-xl hover:scale-105 flex-shrink-0 w-20 sm:w-24 ${
                                                            isSelected 
                                                    ...
                                                    >
                                                        <div className={`relative bg-white rounded-xl overflow-hidden border-2 transition-colors ${
                                                            ...
                                                            <div className="aspect-square relative">
                                                                ...
                                                                <button
                                                                    ...
                                                                    className="absolute top-1 left-1 sm:top-2 sm:left-2 opacity-0 group-hover:opacity-100 transition-opacity bg-black/60 hover:bg-black/80 text-white p-1 sm:p-1.5 rounded-lg"
                                                                    title="View product"
                                                                >
                                                                    <svg className="w-2.5 h-2.5 sm:w-3 sm:h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                            ...
                                                            <div className="px-1 sm:px-2 py-1.5 sm:py-2">
                                                                <div className="text-xs sm:text-sm font-semibold text-gray-800 truncate text-center">
                                                                    {item.price}
                                                                </div>
                                                            </div>
                                                        </div>
                                                        ...
                                                        <div className="hidden sm:block absolute -top-12 left-1/2 transform -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none z-20">
    ...
    const GenerateFromProductsUI = () => {
        ...
        return (
            <div>
                <div className="bg-white rounded-2xl shadow-lg border border-gray-100 w-full max-w-full mt-6 p-4 sm:p-6">
                    <div className="flex items-center justify-between gap-4 sm:gap-8">
                        ...
                        <button 
                            ...
                            className={`group relative px-4 sm:px-6 py-2.5 sm:py-3 rounded-xl font-medium text-white transition-all duration-300 transform flex items-center space-x-2 shadow-lg text-sm sm:text-base ${
                                isGeneratingImage
                ...
                {isGeneratingImage && (
                    <div className="mt-4 p-3 sm:p-4 bg-blue-50 rounded-xl w-full max-w-full border border-blue-200">
                        <div className="flex items-center space-x-3 mb-2">
                            <div 
                                className="w-4 sm:w-5 h-4 sm:h-5 border-2 border-t-transparent rounded-full animate-spin"
    ...
    const GeneratedImagesUI = () => {
        ...
        return (
            <div className="mt-6 p-4 sm:p-6 bg-white rounded-2xl shadow-lg border border-gray-100 w-full max-w-full">
                <h3 className="text-lg font-semibold text-gray-800 mb-4 flex items-center">
                    ...
                    {generatedImages.map((image, index) => (
                        <div key={image.id} className="relative group">
                            <div className="relative bg-white rounded-xl overflow-hidden border border-gray-200 hover:border-gray-300 transition-colors shadow-sm hover:shadow-md">
                                ...
                                <div className="absolute top-2 sm:top-3 right-2 sm:right-3 opacity-0 group-hover:opacity-100 transition-opacity flex space-x-1">
                                    <button
                                        onClick={() => setExpandedImage(`data:image/png;base64,${image.base64Image}`)}
                                        className="bg-white/90 hover:bg-white text-gray-700 p-1.5 sm:p-2 rounded-lg shadow-sm"
                                        title="Expand image"
                                    >
                                    ...
                                    <button
                                        ...
                                        className="bg-red-50 hover:bg-red-100 text-red-600 p-1.5 sm:p-2 rounded-lg shadow-sm"
                                        title="Delete image"
                                    >
                                ...
                                {/* Image number badge */}
                                <div className="absolute bottom-2 sm:bottom-3 left-2 sm:left-3 bg-white/90 text-gray-700 text-xs sm:text-sm px-2 sm:px-3 py-1 rounded-lg shadow-sm">
                                    #{index + 1}
                                </div>
                            </div>
                        </div>
    ...
    return (
        <div className="flex h-screen h-[100dvh] bg-gray-50 overflow-y-hidden">
                ...
                <div className="p-6 border-b border-gray-200">
                    ...
                    <div className="flex gap-2">
                        <button
                            ...
                            className="flex-1 bg-gradient-to-r from-yellow-400 to-yellow-500 hover:from-yellow-500 hover:to-yellow-600 text-white px-4 py-3 rounded-xl font-medium transition-all duration-300 transform hover:scale-105 flex items-center justify-center space-x-2 shadow-md hover:shadow-lg"
                        >
            ...
            <div className="flex-1 flex flex-col">
                ...
                {/* Messages Area */}
                <div className="overflow-y-auto px-4 sm:px-6 py-4 sm:py-6 space-y-4 sm:space-y-6 pb-safe h-full">
                    {currentSession?.conversation.length === 0 && !isGenerating ? (
                        <div className="flex items-center justify-center h-full">
                            <div className="text-center max-w-md px-4">
                                <div 
                                    className="w-16 sm:w-20 h-16 sm:h-20 mx-auto rounded-2xl flex items-center justify-center mb-4 sm:mb-6 text-white"
                                    style={{ background: 'linear-gradient(135deg, #5045e6, #7c3aed)' }}
                                >
                                    <Bot className="w-8 sm:w-10 h-8 sm:h-10" />
                                </div>
                                <h3 className="text-xl sm:text-2xl font-semibold mb-3 sm:mb-4 text-gray-800">Start a New Conversation</h3>
                                <p className="text-gray-600 leading-relaxed text-sm sm:text-base">Hi! I'm Saumya, your AI room design companion. Describe what you want to create and I'll help bring your vision to life.</p>
                            </div>
                        </div>
                    ) : (
                        <>
                            {currentSession?.conversation.map((message, index) => {
                                ...
                                return (
                                    <div key={index} className={`flex ${message.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                                        <div className={`flex items-start gap-3 sm:gap-4 max-w-full sm:max-w-3xl ${message.role === 'user' ? 'flex-row-reverse' : ''}`}>
                                            {/* Avatar */}
                                            ...
                                            <div className={`flex flex-col ${message.role === 'user' ? 'items-end' : 'items-start'} min-w-0 flex-1`}>
                                                <div className={`px-3 sm:px-5 py-3 sm:py-4 rounded-2xl max-w-full shadow-sm ${
                                                    message.role === 'user'
                                                        ? 'text-white'
                                                ...
                                                style={{
                                                        ...
                                                        <div className="mb-3 sm:mb-4 space-y-2">
                                                            {messageImages.map((imageSrc, imgIndex) => (
                                                                <img
                                                                    ...
                                                                    src={imageSrc}
                                                                    alt="Uploaded image"
                                                                    className="max-w-full sm:max-w-md rounded-xl shadow-sm"
                                                                />
                                                            ))}
                                                        </div>
                                                    )}
                                                    ...
                                                    {/* Message Text */}
                                                    <pre className="text-sm leading-relaxed whitespace-pre-wrap break-words">{messageText}</pre>
                                                </div>
                            ...
                            {isGenerating && (
                                <div className="flex justify-start">
                                    <div className="flex items-start space-x-3 sm:space-x-4 max-w-full sm:max-w-3xl">
                                        {/* Avatar */}
                                        <div 
                                            className="w-8 sm:w-10 h-8 sm:h-10 rounded-full flex items-center justify-center flex-shrink-0 text-white"
                                            style={{ background: 'linear-gradient(135deg, #5045e6, #7c3aed)' }}
                                        >
                                            <Bot className="w-4 sm:w-5 h-4 sm:h-5" />
                                        </div>
                                        ...
                                        <div className="flex flex-col items-start">
                                            <div className="bg-white rounded-2xl p-4 sm:p-6 w-full max-w-md border border-gray-200 shadow-sm">
                                                <div className="flex items-center space-x-3 mb-4">
                                                    <div 
                                                        className="w-5 sm:w-6 h-5 sm:h-6 border-2 border-t-transparent rounded-full animate-spin"
                                                        style={{ borderColor: '#5045e6', borderTopColor: 'transparent' }}
                                                    ></div>
                ...
                {/* Input Area */}
                <div className="border-t border-gray-200 bg-white sticky bottom-0 supports-[height:100dvh]:relative">
                    <div className="p-4 sm:p-6 pb-[calc(1rem+env(safe-area-inset-bottom))] sm:pb-6">
                        <div className="max-w-4xl mx-auto">
                            {/* Upload Preview */}
                            {uploadedImage && (
                                <div className="mb-4 p-3 sm:p-4 bg-gray-50 rounded-xl flex items-center space-x-3 sm:space-x-4 border border-gray-200">
                                    <img 
                                        src={URL.createObjectURL(uploadedImage)} 
                                        alt={uploadedImage.name}
                                        className="w-12 sm:w-14 h-12 sm:h-14 object-cover rounded-lg"
                                    />
                                    <div className="flex-1 min-w-0">
                                        <p className="text-sm font-medium text-gray-800 truncate">{uploadedImage.name}</p>
                                ...
                                <div className="flex items-end space-x-3 sm:space-x-4">
                                    <div className="flex-1 relative">
                                        <textarea
                                            ...
                                            className="w-full bg-transparent text-gray-800 placeholder-gray-500 resize-none outline-none text-sm leading-relaxed min-h-[50px] sm:min-h-[60px] max-h-32 pr-10 sm:pr-12"
                                            rows={2}
                                        />
                                        <button
                                            onClick={() => fileInputRef.current?.click()}
                                            className="absolute bottom-2 sm:bottom-3 right-2 sm:right-3 p-1.5 sm:p-2 hover:bg-gray-100 rounded-lg transition-colors text-gray-500"
                                        >
                                            <ImagePlus className="w-4 sm:w-5 h-4 sm:h-5" />
                                        </button>
                                    </div>
                                    ...
                                    <button
                                        ...
                                        className={`text-white p-2.5 sm:p-3 rounded-xl transition-all duration-200 flex-shrink-0 shadow-md hover:shadow-lg ${
                                ...
                                {[
                                    ...
                                    <div key={index} className="relative group">
                                        <button
                                            ...
                                            className="relative w-12 sm:w-16 h-8 sm:h-12 bg-white hover:bg-gray-50 rounded-lg sm:rounded-xl overflow-hidden transition-all duration-200 hover:scale-105 border border-gray-200 hover:border-gray-300 shadow-sm hover:shadow-md"
                                        >
                                            <img
                                                src={room.url}
                                                alt={room.name}
                                        ...
                                        {/* Hover enlarged image - Desktop only */}
                                        <div className="hidden lg:block absolute bottom-12 left-1/2 transform -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none z-20" style={{width: '280px'}}>
